...
export default function OnboardingScreen() {
  const { token, rehydrate, user } = useAuth();
  const router = useRouter();

  // Reidrata o token ao abrir o app
  useEffect(() => {
    rehydrate();
  }, []);
  ...
  const handleLogin = () => {
    if (token && user) {
      router.replace('/(tabs)/dashboard'); // Redireciona para o dashboard, se o usuário já estiver autenticado
    } else {
      router.replace('/(auth)/login'); // Redireciona para o login se o usuário não estiver autenticado
    }
  };
    ...
    <View style={styles.container}>
      <View style={styles.formContainer}>
        ...
        <View style={styles.form}>
          <TouchableOpacity 
            onPress={handleLogin} // Navegação ao clicar no botão
